...
def stack_images(image_file_base, outfile):
	num_to_stack = 1
	while path.exists(image_file_base + str(num_to_stack) + ".fits") or path.exists(image_file_base + '0' + str(num_to_stack) + ".fits"):
		num_to_stack += 1
	
	print 'stacking: ' + str(num_to_stack)
	
	image_list = [ fits.open(image_file_base + str(n) + ".fits") if n > 9 else fits.open(image_file_base + '0' + str(n) + ".fits") for n in range(1, num_to_stack) ]
